...
TRACE = Path("data/traces/moe_Qwen3-30B-A3B_xdoc.npz")
...
def load():
    ...
    # per (layer, doc) count vector, and the per-(layer) ordered eval token stream builder
    idx = {L: d[f"L{L}_idx"] for L in layers}
    k = int(idx[layers[0]].shape[1])
    # counts[L][doc] = (n_slots,) access counts
...
def main():
    ...
    # --- all C(8,4)=70 calib/eval partitions: CV band ---
    hits, ratios = [], []
    for cal in itertools.combinations(docs, len(docs) // 2):
        ev = [d for d in docs if d not in cal]
        hits.append(heldout_hit_at_m(layers, counts, list(cal), ev, m8))
        c = heldout_curve(layers, counts, list(cal), ev, n_slots)
        ratios.append(iso_ratio(c, n_slots)[0])
    ...
    print(f"[all 70 calib/eval splits] held-out hit@8% : median {h_md:.3f}  95% CV band [{h_lo:.3f}, {h_hi:.3f}]")
    print(f"[all 70 calib/eval splits] iso-0.95 ratio  : median {r_md:.2f}x 95% CV band [{r_lo:.2f}, {r_hi:.2f}]x")

    # --- doc-level bootstrap over the 4 canonical eval docs (reference; coarse at n=4) ---
    rng = np.random.default_rng(0); B = 2000; bh = []
    for _ in range(B):
        samp = list(rng.choice(ev0, size=len(ev0), replace=True))
        bh.append(heldout_hit_at_m(layers, counts, cal0, samp, m8))
    bh = np.array(bh)
    print(f"[bootstrap eval docs n=4]  held-out hit@8% : 95% CI [{np.percentile(bh,2.5):.3f}, {np.percentile(bh,97.5):.3f}]\n")

    # --- dynamic-policy baselines on the held-out (eval) stream ---
    print("Dynamic vs static, scored on the SAME held-out eval stream (4 docs):")
    ...
    out = Path("data/eval/residency_cis_lru.csv")
